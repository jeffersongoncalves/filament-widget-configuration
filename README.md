...
![Filament Widget Configuration](https://raw.githubusercontent.com/jeffersongoncalves/filament-widget-configuration/2.x/art/jeffersongoncalves-filament-widget-configuration.png)
...
[![GitHub Tests Action Status](https://img.shields.io/github/actions/workflow/status/jeffersongoncalves/filament-widget-configuration/tests.yml?branch=2.x&label=tests&style=flat-square)](https://github.com/jeffersongoncalves/filament-widget-configuration/actions?query=workflow%3Atests+branch%3A2.x)
[![GitHub Code Style Action Status](https://img.shields.io/github/actions/workflow/status/jeffersongoncalves/filament-widget-configuration/pint.yml?branch=2.x&label=code%20style&style=flat-square)](https://github.com/jeffersongoncalves/filament-widget-configuration/actions?query=workflow%3A"Fix+PHP+code+styling"+branch%3A2.x)
...
On Filament 4/5, `$isLazy` (`CanBeLazy`) is still a **static** property, so it's overridden once, at boot time, on Filament's base `Widget` class — every subclass that hasn't redeclared it inherits the new default for free. `$pollingInterval` (`CanPoll`), however, became a **non-static instance property**, so it can no longer be pre-set for an entire class hierarchy; instead, the plugin registers a [Livewire `ComponentHook`](https://livewire.laravel.com/docs/hooks) that applies the resolved default to each widget instance individually, on `mount`/`hydrate`, using the same declaring-class check per instance.
...
```bash
composer require jeffersongoncalves/filament-widget-configuration:"^2.0"
```
